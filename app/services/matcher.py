...
async def run_match_analysis(analysis_id: UUID) -> None:
    """Full-depth match only (legacy / manual re-analyze)."""
...
async def run_progressive_match_analysis(analysis_id: UUID) -> None:
    """M5: fast screen result first, then replace with full analysis."""
    async with async_session() as db:
        analysis = await db.get(MatchAnalysis, analysis_id)
        ...
        try:
            ...
            analysis.result = _screen_result_payload(screen_match)
            analysis.error = None
            await db.commit()
            logger.info(
